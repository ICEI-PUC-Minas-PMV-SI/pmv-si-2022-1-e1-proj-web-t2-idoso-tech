...
const mostrarEsconderAvaliacao = () => {
  ...
  carregarUsuario(1).then((usuario) => {
    if (usuario) {
...
const pegarVideo = async (id) => {
  ...
  return fetch(`http://localhost:3000/videos/${id}`)
    ...
    .catch((err) => {
      ...
      mostrarMensagem(
        "mensagem",
        "Ops! Não foi possível carregar o vídeo.",
        "erro"
      );
    });
};
const atualizarVideo = (video) => {
  ...
  return fetch(`http://localhost:3000/videos/${video.id}`, {
    ...
    .catch((erro) => {
      console.log(
        `Falha para atualizar o vídeo id ${id} video ${video}. Erro ${erro}`
      );
...
const atualizarUsuario = (usuario) => {
  return fetch(`http://localhost:3000/usuarios/${usuario.id}`, {
    ...
    .catch((erro) => {
      console.log(
        `Falha para atualizar o usuario id ${id} usuario ${usuario}. ${erro}`
      );
    });
};
...
const avaliarVideo = (nota, event) => {
  ...
  pegarVideo(videoId).then((video) => {
    try {
      ...
      mostrarMensagem(
        "mensagem",
        "Avaliar video falhou, tente novamente depois.",
        "erro"
      );
      return;
    }
    ...
    atualizarVideo(video).then((response) => {
      if (response.ok) {
